    ...
    for loc,row in trans.iterrows():
        ...
        for i in gene_tree:
            if define_sub(i.name)=="OUT" or i.name in outleaf:
                i.delete()
        # n_dist=dict((i.name,i.dist)for i in gene_tree.get_leaves())
        for i in HGT_gene:
            # print(i)
            try:
            ...
            if cur_set is not None:
                ...
                continue
        #define donor gene according to genetic distance
        #gene with closest distance to HGT gene was disnated as donor gene
        give_gene_dist=dict((x,statistics.median([(gene_tree&x).get_distance(i) for i in HGT_gene]))for x in give_gene )
        donor_gene=min(give_gene_dist.keys(),key=give_gene_dist.get)
        print(og_id,str(loc),define_sub(donor_gene),cur_sub,sep="\t")
        with open(og_id+'_'+str(loc)+"_id",'w') as id_file:
            for i in HGT_gene:
                id_file.writelines(i+'\n')
            id_file.writelines(donor_gene+'\n')
